...
namespace ModelPipeline
{
		...
		: mModel(&model), mBone(nullptr), mKeyframes()
    {
		...
		mBone = model.Bones().at(boneIndex).get();
		...
		for (UINT i = 0; i < nodeAnim.mNumPositionKeys; i++)
		{
			...
			std::shared_ptr<Keyframe> keyframe (new Keyframe(static_cast<float>(positionKey.mTime), XMFLOAT3(positionKey.mValue.x, positionKey.mValue.y, positionKey.mValue.z),
