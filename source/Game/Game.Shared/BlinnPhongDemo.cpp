...
namespace RenderGame
{
	...
	BlinnPhongDemo::BlinnPhongDemo(const Library::Game& game, const std::shared_ptr<Camera>& camera)
	...
	void BlinnPhongDemo::Initialize()
	{
		...
		std::string compiledVertexShader = Utility::LoadBinaryFile("Content\\Shaders\\BlinnPhongDemoVS.cso");
		...
		std::string compiledPixelShader = Utility::LoadBinaryFile("Content\\Shaders\\BlinnPhongDemoPS.cso");
	...
	void BlinnPhongDemo::Render(const GameTime& gameTime)
	{
	...
	void BlinnPhongDemo::CreateVertexBuffer(ID3D11Device* device, const std::shared_ptr<Mesh>& mesh, ID3D11Buffer** vertexBuffer) const
	{
