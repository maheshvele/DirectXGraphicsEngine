...
namespace Library
{
	RTTI_DEFINITIONS(AnimationPlayer)
		...
		AnimationPlayer::AnimationPlayer(const Game& game, Model& model, bool interpolationEnabled)
		: GameComponent(game),
	...
	void AnimationPlayer::StartClip(AnimationClip& clip)
	{
		...
		XMVECTOR vector = XMMatrixDeterminant(mModel->RootNode()->TransformMatrix());
		XMMATRIX matrix = mModel->RootNode()->TransformMatrix();

		XMMATRIX inverseRootTransform = XMMatrixInverse(&vector, matrix);
	...
	void AnimationPlayer::Update(const GameTime& gameTime)
	{
		if (mIsPlayingClip)
		{
			...
			mCurrentTime += static_cast<float>(gameTime.GetElapsedSeconds()) * mCurrentClip->TicksPerSecond();
	...
	void AnimationPlayer::GetBindPoseBottomUp(SceneNode& sceneNode)
	{
		...
		SceneNode* parentNode = sceneNode.GetParentNode();
	...
	void AnimationPlayer::GetBindPose(SceneNode& sceneNode)
	{
		...
		XMMATRIX toRootTransform = (sceneNode.GetParentNode() != nullptr ? toParentTransform * XMLoadFloat4x4(&(mToRootTransforms.at(sceneNode.GetParentNode()))) : toParentTransform);
	...
	void AnimationPlayer::GetPose(float time, SceneNode& sceneNode)
	{
		...
		XMMATRIX toRootTransform = (sceneNode.GetParentNode() != nullptr ? XMLoadFloat4x4(&toParentTransform) * XMLoadFloat4x4(&(mToRootTransforms.at(sceneNode.GetParentNode()))) : XMLoadFloat4x4(&toParentTransform));
		XMStoreFloat4x4(&(mToRootTransforms[&sceneNode]), toRootTransform);
